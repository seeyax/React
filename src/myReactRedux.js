...
// Provider 就是把传递进来的store放在根组件的上下文中
...
/** connect：获取上下文中的store，把公共状态、要派发的方法等，要基于属性传递给需要渲染的组件：
...
export function connect(mapStateToProps, mapDispatchToProps) {
  ...
  return function currying(Component) {
    ...
    return function HOC(props) {
      ...
      let state = getState()
      let nextState = useMemo(() => { return mapStateToProps(state) }, [state])
